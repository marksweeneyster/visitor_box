...
public:
  ...
  void operator()(const Lorikeet& l) {
    std::cout << "Feed grain to the " << l.color() << " bird" << std::endl;
  }
};
...
public:
  ...
  void operator()(const Lorikeet& l) {
    std::cout << "Teach words to the " << l.color() << " bird" << std::endl;
  }
};
...
void walk(const Pet& p) {
  ...
  public:
    ...
    void operator()(const Dog& d) {
      std::cout << "Take the " << d.color() << " dog for a walk" << std::endl;
    }
    void operator()(const Lorikeet& l) {
      std::cout << "Release the " << l.color() << " bird to fly" << std::endl;
    }
  } v;
  std::visit(v, p);
}

...
auto fv = [](const auto& p) {
  if constexpr (SAME(p, Cat)) {
    std::cout << "Feed tuna to the " << p.color() << " cat" << std::endl;
  } else if constexpr (SAME(p, Dog)) {
    std::cout << "Feed steak to the " << p.color() << " dog" << std::endl;
  } else if constexpr (SAME(p, Lorikeet)) {
    std::cout << "Feed grain to the " << p.color() << " bird" << std::endl;
  } else
    abort();
};
#undef SAME

auto pv = overloaded{[](const Cat& c) {
...
void walk(const Pet& p) {
  auto v = overloaded{[](const Cat& c) {
                        std::cout << "Let the " << c.color() << " cat out"
                                  << std::endl;
                      },
                      [](const Dog& d) {
                        std::cout << "Take the " << d.color()
                                  << " dog for a walk" << std::endl;
                      },
                      [](const Lorikeet& l) {
                        std::cout << "Release the " << l.color()
                                  << " bird to fly" << std::endl;
                      }};
  std::visit(v, p);
}
